...
var PluggableComboChart = /** @class */ (function (_super) {
    ...
    PluggableComboChart.prototype.getDefaultAndAvailableSort = function (buckets) {
        var measures = getBucketItemsByType(buckets, BucketNames.MEASURES, [METRIC]);
        var secondaryMeasures = getBucketItemsByType(buckets, BucketNames.SECONDARY_MEASURES, [METRIC]);
        ...
        if (!isEmpty(viewBy) && (!isEmpty(measures) || !isEmpty(secondaryMeasures))) {
            var mergedMeasures = __spreadArrays(measures, secondaryMeasures);
            return {
                defaultSort: defaultSort,
                availableSorts: [
                    newAvailableSortsGroup(viewBy[0].localIdentifier, mergedMeasures.map(function (m) { return m.localIdentifier; }), true, mergedMeasures.length > 1),
                ],
            };
        }
        return {
    ...
    PluggableComboChart.prototype.isSortDisabled = function (referencePoint, availableSorts) {
        var buckets = referencePoint.buckets;
        var measures = getBucketItemsByType(buckets, BucketNames.MEASURES, [METRIC]);
        var secondaryMeasures = getBucketItemsByType(buckets, BucketNames.SECONDARY_MEASURES, [METRIC]);
        var viewBy = getBucketItems(buckets, BucketNames.VIEW);
        var disabled = viewBy.length < 1 ||
            availableSorts.length === 0 ||
            (measures.length < 1 && secondaryMeasures.length < 1);
        var disabledExplanation = getCustomSortDisabledExplanation(__spreadArrays(measures, secondaryMeasures), viewBy, this.intl);
        return {
            disabled: disabled,
            disabledExplanation: disabledExplanation,
        };
    };
    PluggableComboChart.prototype.getSortConfig = function (referencePoint) {
        ...
        var _a = this.getDefaultAndAvailableSort(buckets), defaultSort = _a.defaultSort, availableSorts = _a.availableSorts;
    ...
    PluggableComboChart.prototype.renderConfigurationPanel = function (insight) {
        var configPanelElement = this.getConfigPanelElement();
