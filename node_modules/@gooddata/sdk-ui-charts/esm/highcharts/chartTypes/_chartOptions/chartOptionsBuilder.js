...
import { isAreaChart, isBubbleChart, isChartSupported, isComboChart, isHeatmap, isFunnel, isPieOrDonutChart, isOneOfTypes, isScatterPlot, isTreemap, stringifyChartTypes, unwrap, } from "../_util/common";
...
function getCategories(type, measureGroup, viewByAttribute, stackByAttribute, emptyHeaderTitle) {
    ...
    if (isHeatmap(type)) {
        return [
            viewByAttribute
                ? viewByAttribute.items.map(function (item) {
                    return valueWithEmptyHandling(item.attributeHeaderItem.name, emptyHeaderTitle);
                })
                : [""],
            stackByAttribute
                ? stackByAttribute.items.map(function (item) {
                    return valueWithEmptyHandling(item.attributeHeaderItem.name, emptyHeaderTitle);
                })
                : [""],
        ];
    ...
    if (viewByAttribute) {
        return viewByAttribute.items.map(function (item) {
            return valueWithEmptyHandling(item.attributeHeaderItem.name, emptyHeaderTitle);
        });
    }
    if (isOneOfTypes(type, multiMeasuresAlternatingTypes)) {
        ...
        return measureGroup.items.map(function (wrappedMeasure) {
...
export function getChartOptions(dataView, chartConfig, drillableItems, emptyHeaderTitle, theme) {
    var _a, _b, _c;
    ...
    var xAxes = getXAxes(dv, config, measureGroup, viewByAttribute, viewByParentAttribute);
    var yAxes = getYAxes(dv, config, measureGroup, stackByAttribute);
    var seriesWithoutDrillability = getSeries(dv, measureGroup, viewByAttribute, stackByAttribute, type, colorStrategy, emptyHeaderTitle, theme);
    ...
    var tooltipFactory = getTooltipFactory(isViewByTwoAttributes, viewByAttribute, viewByParentAttribute, stackByAttribute, measure, emptyHeaderTitle, config, isDualAxis);
    return {
        type: type,
        stacking: stacking,
