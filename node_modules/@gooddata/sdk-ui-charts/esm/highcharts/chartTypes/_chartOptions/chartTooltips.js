...
export function generateTooltipHeatmapFn(viewByAttribute, stackByAttribute, emptyHeaderTitle, config) {
    ...
    return function (point, maxTooltipContentWidth) {
        ...
        if (viewByAttribute) {
            textData.unshift([
                customEscape(viewByAttribute.formOf.name),
                customEscape(valueWithEmptyHandling(viewByAttribute.items[point.x].attributeHeaderItem.name, emptyHeaderTitle)),
            ]);
        }
        if (stackByAttribute) {
            textData.unshift([
                customEscape(stackByAttribute.formOf.name),
                customEscape(valueWithEmptyHandling(stackByAttribute.items[point.y].attributeHeaderItem.name, emptyHeaderTitle)),
            ]);
        }
        return renderTooltipHTML(textData, maxTooltipContentWidth);
    };
...
export function getTooltipFactory(isViewByTwoAttributes, viewByAttribute, viewByParentAttribute, stackByAttribute, measure, emptyHeaderTitle, config, isDualAxis) {
    if (config === void 0) { config = {}; }
    if (isDualAxis === void 0) { isDualAxis = false; }
