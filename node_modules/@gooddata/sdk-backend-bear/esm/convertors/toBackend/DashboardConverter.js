...
export var convertWidget = function (widget) {
    ...
    if (widget.type === "kpi") {
        ...
        return {
            kpi: {
                content: __assign(__assign({}, (isKpiWithComparison(kpi)
                    ? {
    ...
    var insight = widget.insight, _a = widget.properties, widgetProperties = _a === void 0 ? {} : _a, configuration = widget.configuration;
    ...
    return {
        visualizationWidget: {
            content: __assign(__assign(__assign({ visualization: refToUri(insight), ignoreDashboardFilters: convertedIgnoredDashboardFilters, dateDataSet: convertedDateDataSet, drills: drills
                    ? drills.map(convertDrill)
                    : [] }, (!isEmpty(nonEmptyProperties) && {
                properties: serializeProperties(nonEmptyProperties),
            })), (!isEmpty(references) && { references: references })), (configuration ? { configuration: configuration } : {})),
            meta: meta,
        },
    };
};
...
export var convertScheduledMailAttachment = function (scheduledMailAttachment) {
    if (isDashboardAttachment(scheduledMailAttachment)) {
        ...
        var widgetDashboard = scheduledMailAttachment.widgetDashboard, widget = scheduledMailAttachment.widget, filterContext = scheduledMailAttachment.filterContext, formats = scheduledMailAttachment.formats, exportOptions = scheduledMailAttachment.exportOptions;
        return {
