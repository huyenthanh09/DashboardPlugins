...
/* 1441px, 1920px */
/* 1921px, ... */
.attribute-filter-button.is-draggable, .attribute-filter-button.is-dragging {
  cursor: grab;
}
.attribute-filter-button.is-draggable::after, .attribute-filter-button.is-dragging::after {
  left: -10px;
}
...
.attribute-filter-button:hover .drag-handle-icon, .attribute-filter-button.is-dragging .drag-handle-icon {
  opacity: 1;
}
...
.drag-info-placeholder-box {
  ...
  padding: 40px;
  border-radius: 15px;
  border: 2px dashed var(--gd-palette-complementary-3, rgba(176, 190, 202, 0.45));
  background: var(--gd-palette-complementary-1, rgba(201, 213, 224, 0.05));
  text-align: center;
}
