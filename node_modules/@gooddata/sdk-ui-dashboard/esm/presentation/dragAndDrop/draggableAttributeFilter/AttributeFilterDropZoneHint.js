...
export function AttributeFilterDropZoneHint(_a) {
    ...
    var _d = __read(useDashboardDrop(acceptPlaceholder ? ["attributeFilter", "attributeFilter-placeholder"] : "attributeFilter", {
        ...
        drop: function (item) {
            var targetIndexPositionCorrection = placement === "inside" && hintPosition === "next" ? 1 : 0;
            if (isAttributeFilterDraggableItem(item)) {
                var identifier = item.filter.attributeFilter.localIdentifier;
                var originalIndex = item.filterIndex;
                var originalPositionCorrection = originalIndex < targetIndex ? -1 : 0;
                var index = targetIndex + targetIndexPositionCorrection + originalPositionCorrection;
                dispatch(moveAttributeFilter(identifier, index));
            }
            if (isAttributeFilterPlaceholderDraggableItem(item) && onAddAttributePlaceholder) {
                var index = targetIndex + targetIndexPositionCorrection;
                return onAddAttributePlaceholder(index);
            }
        },
    }, [inactiveIndexes, targetIndex, placement, hintPosition]), 2), _e = _d[0], canDrop = _e.canDrop, isOver = _e.isOver, dropRef = _d[1];
