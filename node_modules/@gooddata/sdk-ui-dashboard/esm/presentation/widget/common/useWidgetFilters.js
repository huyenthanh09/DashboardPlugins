...
export function useWidgetFilters(widget, insight) {
    ...
    return {
        ...
        status: combineQueryProcessingStatuses(nonIgnoredFiltersStatus, effectiveFiltersState.filterQueryStatus, status),
...
function useNonIgnoredFilters(widget) {
    ...
    }, [safeSerializeObjRef(widget === null || widget === void 0 ? void 0 : widget.ref), filtersDigest(dashboardFilters, widgetIgnoresDateFilter)]);
...
function combineQueryProcessingStatuses() {
    ...
    return ((_a = flow(compact, sortBy(function (status) { return statusPriorities[status]; }), first)(statuses)) !== null && _a !== void 0 ? _a : "pending");
}
/**
...
function filtersDigest(filters, ignoreDateFilter) {
    return filters
        ...
        if (isDashboardAttributeFilter(filter)) {
            ...
            var isNoop = filter.attributeFilter.negativeSelection &&
...
function fullFiltersDigest(filters) {
    return filters
        .map(function (filter) { return stringify(filter); })
        .sort()
        .join("|");
}
