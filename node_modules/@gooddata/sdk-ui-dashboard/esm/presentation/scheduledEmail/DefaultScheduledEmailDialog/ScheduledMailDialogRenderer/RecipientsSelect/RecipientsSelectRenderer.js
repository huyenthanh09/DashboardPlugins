...
var MAXIMUM_RECIPIENTS_RECEIVE = 60;
var DELAY_TIME = 500;
var PADDING = 16;
...
var bubbleAlignPoints = [{ align: "cr cl" }];
var RecipientsSelectRenderer = /** @class */ (function (_super) {
    __extends(RecipientsSelectRenderer, _super);
    function RecipientsSelectRenderer() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.recipientRef = React.createRef();
        _this.renderEmptyContainer = function () {
            return null;
        };
        _this.renderNoOptionsContainer = function (commonProps) {
            var inputValue = commonProps.selectProps.inputValue;
            if (inputValue) {
        ...
        _this.renderMenuOptions = function (menuProps) {
            var _a = _this.props, isLoading = _a.isLoading, author = _a.author;
            ...
            var areAllValuesSelected = options.length &&
                options.every(function (option) {
                    return (isScheduleEmailExistingRecipient(option) && isEqual(option, author)) ||
        ...
        _this.renderOwnerValueContainer = function (name, email) {
            ...
            var value = _this.currentUserIsAuthor() ? name : email;
        ...
        _this.renderErrorValueContainer = function (label, removeIcon, bubbleMessageTranslationId) {
        ...
        _this.renderSingleValueContainer = function (singleValueProps) {
            ...
            return _this.renderOwnerValueContainer(displayName, email);
        };
        _this.renderMultiValueContainer = function (multiValueProps) {
            ...
            if (isScheduleEmailExistingRecipient(data) && isEqual(data, _this.props.author)) {
                var displayName = getScheduledEmailRecipientDisplayName(data);
                var email_1 = getScheduledEmailRecipientEmail(data);
                return _this.renderOwnerValueContainer(displayName, email_1);
            }
            var email = getScheduledEmailRecipientEmail(data);
            if (!isEmail(email)) {
                return _this.renderErrorValueContainer(email, removeIcon, messages.scheduleEmailOptionRecepientInvalid.id);
            }
            // don't allow adding external recipients to schedules created by somebody else than the current user
            if (!_this.currentUserIsAuthor() &&
                isScheduleEmailExternalRecipient(data) &&
                !_this.isOriginalExternalRecipient(data)) {
                return _this.renderErrorValueContainer(email, removeIcon, messages.scheduleEmailOptionRecepientExternalNotAllowed.id);
            }
        ...
        _this.handleOnChange = function (selectedValues, actionTypes) {
            ...
            var _j = _this.props, value = _j.value, allowEmptySelection = _j.allowEmptySelection;
            ...
            if (selectedValues === null) {
                if (allowEmptySelection) {
                    ...
                    (_f = (_e = _this.props).onChange) === null || _f === void 0 ? void 0 : _f.call(_e, [value[0]]);
                }
                return;
            }
            (_h = (_g = _this.props).onChange) === null || _h === void 0 ? void 0 : _h.call(_g, selectedValues);
        };
        ...
        _this.onSearchCore = function (searchString) {
            _this.loadUserListItems(searchString);
        };
        _this.onSearch = debounce(_this.onSearchCore, DELAY_TIME);
    ...
    RecipientsSelectRenderer.prototype.componentDidMount = function () {
    ...
    RecipientsSelectRenderer.prototype.render = function () {
        var _a = this.props, isMulti = _a.isMulti, options = _a.options, value = _a.value;
    ...
    RecipientsSelectRenderer.prototype.getStyle = function () {
        var current = this.recipientRef.current;
        var width = ((!isEmpty(current) && current.getBoundingClientRect()) || { width: undefined }).width;
        return {
            maxWidth: width
    ...
    RecipientsSelectRenderer.prototype.currentUserIsAuthor = function () {
        var _a = this.props, currentUser = _a.currentUser, author = _a.author;
        return isScheduleEmailExistingRecipient(author) && areObjRefsEqual(author.user.ref, currentUser.ref);
    };
    RecipientsSelectRenderer.prototype.isOriginalExternalRecipient = function (recipient) {
        return this.props.originalValue.some(function (option) {
            return isScheduleEmailExternalRecipient(option) && option.email === recipient.email;
        });
    };
    RecipientsSelectRenderer.prototype.getMatchedRecipientEmails = function (options, searchKey) {
        return searchKey
    ...
    return RecipientsSelectRenderer;
}(React.PureComponent));
export { RecipientsSelectRenderer };
