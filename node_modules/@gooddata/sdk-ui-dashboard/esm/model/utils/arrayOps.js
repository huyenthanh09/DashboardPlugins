...
export function addArrayElements(arr, index, items) {
    if (index === 0) {
        arr.unshift.apply(arr, __spread(items));
    }
    else if (index < 0) {
        arr.push.apply(arr, __spread(items));
    }
    else {
        arr.splice.apply(arr, __spread([index, 0], items));
    }
}
...
export function moveArrayElement(arr, fromIndex, toIndex) {
    var element = removeArrayElement(arr, fromIndex);
