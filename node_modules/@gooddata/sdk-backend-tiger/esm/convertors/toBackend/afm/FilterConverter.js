...
function convertMeasureValueFilter(filter, applyOnResultProp) {
    ...
    if (isComparisonCondition(condition)) {
        ...
        return {
            comparisonMeasureValueFilter: __assign({ measure: toAfmIdentifier(measureValueFilter.measure), 
    ...
    if (isRangeCondition(condition)) {
        ...
        return {
            rangeMeasureValueFilter: __assign({ measure: toAfmIdentifier(measureValueFilter.measure), 
                // Operator has same values, we only need type assertion
                operator: operator, 
...
function convertRankingFilter(filter, applyOnResultProp) {
    ...
    var dimensionalityProp = attributes ? { dimensionality: attributes.map(toAfmIdentifier) } : {};
    return {
        rankingFilter: __assign(__assign(__assign({ measures: [toAfmIdentifier(measure)] }, dimensionalityProp), { operator: operator,
            value: value }), applyOnResultProp),
    };
}
