...
import { createExportErrorFunction, ErrorCodes, ErrorComponent, IntlWrapper, LoadingComponent, newErrorMapping, } from "@gooddata/sdk-ui";
 ...
 * @internal
 */
var CorePivotTableAgImpl = /** @class */ (function (_super) {
    __extends(CorePivotTableAgImpl, _super);
    function CorePivotTableAgImpl(props) {
         ...
         */
        _this.initialize = function (execution) {
            _this.internal.abandonInitialization();
            var initializer = new TableFacadeInitializer(execution, _this.getTableMethods(), _this.props);
            initializer.initialize().then(function (result) {
                if (!result || _this.internal.initializer !== result.initializer) {
                    /*
                     * This particular initialization was abandoned.
                     */
                    return;
                }
                _this.internal.initializer = undefined;
                _this.internal.table = result.table;
                _this.setState({ readyToRender: true });
            });
            return initializer;
        };
        /**
         ...
         */
        _this.reinitialize = function (execution) {
            _this.setState({
                readyToRender: false,
                columnTotals: cloneDeep(sanitizeDefTotals(execution.definition)),
                error: undefined,
                desiredHeight: _this.props.config.maxHeight,
                resized: false,
            }, function () {
                _this.internal.destroy();
                _this.internal = new InternalTableState();
                _this.boundAgGridCallbacks = _this.createBoundAgGridCallbacks();
                _this.internal.initializer = _this.initialize(execution);
            });
        };
        //
        // Render
        ...
        _this.setContainerRef = function (container) {
            ...
            if (_this.containerRef) {
                _this.containerRef.addEventListener("mousedown", _this.onContainerMouseDown);
            }
        };
        //
        // event handlers
        //
        _this.onGridReady = function (event) {
            invariant(_this.internal.table);
            _this.internal.table.finishInitialization(event.api, event.columnApi);
            _this.updateDesiredHeight();
            if (_this.getGroupRows()) {
                _this.internal.table.initializeStickyRow();
            }
            _this.internal.table.setTooltipFields();
        };
        _this.onFirstDataRendered = function (_event) { return __awaiter(_this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        if (!this.isColumnAutoresizeEnabled()) return [3 /*break*/, 2];
                        return [4 /*yield*/, this.autoresizeColumns()];
                    case 1:
                        _a.sent();
                        return [3 /*break*/, 3];
                    case 2:
        ...
        _this.onModelUpdated = function () {
            _this.updateStickyRow();
        };
        _this.onGridColumnsChanged = function () {
            _this.updateStickyRow();
        };
        _this.onGridSizeChanged = function (gridSizeChangedEvent) {
            if (!_this.internal.firstDataRendered) {
            ...
            if (!_this.internal.table) {
                return;
            }
            if (_this.internal.table.isResizing()) {
            ...
            if (_this.internal.checkAndUpdateLastSize(gridSizeChangedEvent.clientWidth, gridSizeChangedEvent.clientHeight)) {
        ...
        _this.onSortChanged = function (event) {
            if (!_this.internal.table) {
                // eslint-disable-next-line no-console
                console.warn("changing sorts without prior execution cannot work");
                return;
            }
            var sortItems = _this.internal.table.createSortItems(event.columnApi.getAllColumns());
            // Changing sort may cause subtotals to no longer be reasonably placed - remove them if that is the case
            var totals = sanitizeDefTotals(_this.getExecutionDefinition(), sortItems);
            // eslint-disable-next-line no-console
            ...
            _this.setState({ columnTotals: totals }, function () {
                var _a;
                (_a = _this.internal.table) === null || _a === void 0 ? void 0 : _a.refreshData();
            });
        };
        _this.onPinnedRowDataChanged = function (event) { return __awaiter(_this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        if (!((event === null || event === void 0 ? void 0 : event.api.getPinnedBottomRowCount()) > 0)) return [3 /*break*/, 2];
                        return [4 /*yield*/, this.autoresizeColumns(true)];
                    case 1:
                        _a.sent();
                        _a.label = 2;
        ...
        _this.onContainerMouseDown = function (event) {
            _this.internal.isMetaOrCtrlKeyPressed = event.metaKey || event.ctrlKey;
            _this.internal.isAltKeyPressed = event.altKey;
        };
        _this.onPageLoaded = function (_dv, newResult) {
            var _a, _b;
            if (!_this.internal.table) {
                return;
            }
            if (newResult) {
                (_b = (_a = _this.props).onExportReady) === null || _b === void 0 ? void 0 : _b.call(_a, _this.internal.table.createExportFunction(_this.props.exportTitle));
            }
            _this.updateStickyRow();
            _this.updateDesiredHeight();
        };
         ...
         */
        _this.onExecutionTransformed = function (_newExecution) {
            if (!_this.internal.table) {
                return;
            }
            _this.internal.table.clearStickyRow();
        };
        _this.onMenuAggregationClick = function (menuAggregationClickConfig) {
            var _a;
            var sortItems = (_a = _this.internal.table) === null || _a === void 0 ? void 0 : _a.getSortItems();
            var newColumnTotals = sanitizeDefTotals(_this.getExecutionDefinition(), sortItems, getUpdatedColumnTotals(_this.getColumnTotals(), menuAggregationClickConfig));
            _this.pushDataGuard({
                properties: {
                    totals: newColumnTotals,
                },
            });
            _this.setState({ columnTotals: newColumnTotals }, function () {
                var _a;
                (_a = _this.internal.table) === null || _a === void 0 ? void 0 : _a.refreshData();
            });
        };
        _this.onLoadingChanged = function (loadingState) {
            var onLoadingChanged = _this.props.onLoadingChanged;
            if (onLoadingChanged) {
                onLoadingChanged(loadingState);
            }
        };
        _this.onError = function (error, execution) {
            var _a, _b;
            if (execution === void 0) { execution = _this.props.execution; }
            var onExportReady = _this.props.onExportReady;
            if (_this.props.execution.fingerprint() === execution.fingerprint()) {
                _this.setState({ error: error.getMessage() });
        ...
        // Table resizing
        //
        _this.growToFit = function () {
            invariant(_this.internal.table);
            if (!_this.isGrowToFitEnabled()) {
                return;
            }
            _this.internal.table.growToFit(_this.getResizingConfig());
            if (!_this.state.resized && !_this.internal.table.isResizing()) {
                _this.setState({
                    resized: true,
                });
            }
        };
        _this.autoresizeColumns = function (force) {
                ...
                return __generator(this, function (_b) {
                    switch (_b.label) {
                        case 0:
                            ...
                            return [4 /*yield*/, ((_a = this.internal.table) === null || _a === void 0 ? void 0 : _a.autoresizeColumns(this.getResizingConfig(), force))];
                        case 1:
                            didResize = _b.sent();
        ...
        _this.shouldAutoResizeColumns = function () {
            var columnAutoresize = _this.isColumnAutoresizeEnabled();
            var growToFit = _this.isGrowToFitEnabled();
        ...
        _this.startWatchingTableRendered = function () {
            ...
            var isTableRendered = _this.shouldAutoResizeColumns()
                ? _this.state.resized
                : _this.internal.table.isPivotTableReady();
            var shouldCallAutoresizeColumns = _this.internal.table.isPivotTableReady() &&
                !_this.state.resized &&
                !_this.internal.table.isResizing();
            if (_this.shouldAutoResizeColumns() && shouldCallAutoresizeColumns) {
        ...
        _this.stopWatchingTableRendered = function () {
            _this.internal.stopWatching();
            _this.props.afterRender();
        };
        //
        // Sticky row handling
        //
        _this.isStickyRowAvailable = function () {
            invariant(_this.internal.table);
            return Boolean(_this.getGroupRows() && _this.internal.table.stickyRowExists());
        };
        _this.updateStickyRow = function () {
            if (!_this.internal.table) {
                return;
            }
            if (_this.isStickyRowAvailable()) {
                _this.internal.table.updateStickyRowPosition();
                var scrollPosition = __assign({}, _this.internal.lastScrollPosition);
                _this.internal.lastScrollPosition = {
                    top: 0,
                    left: 0,
                };
                _this.updateStickyRowContent(scrollPosition);
        ...
        _this.updateStickyRowContent = function (scrollPosition) {
            invariant(_this.internal.table);
            if (_this.isStickyRowAvailable()) {
                _this.internal.table.updateStickyRowContent({
                    scrollPosition: scrollPosition,
                    lastScrollPosition: _this.internal.lastScrollPosition,
                });
            }
            _this.internal.lastScrollPosition = __assign({}, scrollPosition);
        };
        //
        // Desired height updating
        //
        _this.getScrollBarPadding = function () {
            var _a;
            if (!((_a = _this.internal.table) === null || _a === void 0 ? void 0 : _a.isFullyInitialized())) {
                return 0;
            }
            if (!_this.containerRef) {
                return 0;
            ...
            return scrollBarExists(_this.containerRef) ? getScrollbarWidth() : 0;
        };
        _this.calculateDesiredHeight = function () {
            var _a, _b;
            var maxHeight = _this.props.config.maxHeight;
            if (!maxHeight) {
                return;
            }
            var bodyHeight = (_b = (_a = _this.internal.table) === null || _a === void 0 ? void 0 : _a.getTotalBodyHeight()) !== null && _b !== void 0 ? _b : 0;
            var totalHeight = bodyHeight + _this.getScrollBarPadding();
            return Math.min(totalHeight, maxHeight);
        };
        _this.updateDesiredHeight = function () {
            if (!_this.internal.table) {
                return;
            }
            var desiredHeight = _this.calculateDesiredHeight();
            /*
             ...
             */
            var HEIGHT_CHANGE_TOLERANCE = 2;
            if (_this.state.desiredHeight === undefined ||
                (desiredHeight !== undefined &&
                    Math.abs(_this.state.desiredHeight - desiredHeight) > HEIGHT_CHANGE_TOLERANCE)) {
                _this.setState({ desiredHeight: desiredHeight });
            }
        };
        //
        ...
        _this.getExecutionDefinition = function () {
            return _this.props.execution.definition;
        };
        _this.getGroupRows = function () {
            var _a, _b;
            return (_b = (_a = _this.props.config) === null || _a === void 0 ? void 0 : _a.groupRows) !== null && _b !== void 0 ? _b : true;
        };
        _this.getMenuConfig = function () {
            var _a, _b;
            return (_b = (_a = _this.props.config) === null || _a === void 0 ? void 0 : _a.menu) !== null && _b !== void 0 ? _b : {};
        };
        _this.getDefaultWidth = function () {
            return DEFAULT_COLUMN_WIDTH;
        };
        _this.isColumnAutoresizeEnabled = function () {
            var defaultWidth = _this.getDefaultWidthFromProps(_this.props);
         ...
         */
        _this.pushDataGuard = function (data) {
            var _a, _b;
            (_b = (_a = _this.props).pushData) === null || _b === void 0 ? void 0 : _b.call(_a, data);
            /*
        ...
        _this.stateBoundCallback = function (callback) {
            var forInternalState = _this.internal;
            return (function () {
                var args = [];
                for (var _i = 0; _i < arguments.length; _i++) {
                    args[_i] = arguments[_i];
                }
                if (_this.internal !== forInternalState) {
                    return;
                }
         ...
         */
        _this.createBoundAgGridCallbacks = function () {
            var debouncedGridSizeChanged = debounce(_this.stateBoundCallback(_this.onGridSizeChanged), AGGRID_ON_RESIZE_TIMEOUT);
            return {
                onGridReady: _this.stateBoundCallback(_this.onGridReady),
                onFirstDataRendered: _this.stateBoundCallback(_this.onFirstDataRendered),
                onBodyScroll: _this.stateBoundCallback(_this.onBodyScroll),
                onModelUpdated: _this.stateBoundCallback(_this.onModelUpdated),
                onGridColumnsChanged: _this.stateBoundCallback(_this.onGridColumnsChanged),
                onGridColumnResized: _this.stateBoundCallback(_this.onGridColumnResized),
                onSortChanged: _this.stateBoundCallback(_this.onSortChanged),
                onGridSizeChanged: debouncedGridSizeChanged,
                onPinnedRowDataChanged: _this.stateBoundCallback(_this.onPinnedRowDataChanged),
            };
        };
        _this.getTableMethods = function () {
            var _a;
            return __assign({ hasColumnWidths: _this.hasColumnWidths(), getExecutionDefinition: _this.getExecutionDefinition, getMenuConfig: _this.getMenuConfig, getGroupRows: _this.getGroupRows, getColumnTotals: _this.getColumnTotals, getResizingConfig: _this.getResizingConfig, onLoadingChanged: _this.onLoadingChanged, onError: _this.onError, onExportReady: (_a = _this.props.onExportReady) !== null && _a !== void 0 ? _a : noop, pushData: _this.pushDataGuard, onPageLoaded: _this.onPageLoaded, onExecutionTransformed: _this.onExecutionTransformed, onMenuAggregationClick: _this.onMenuAggregationClick }, _this.boundAgGridCallbacks);
        };
        _this.getResizingConfig = function () {
            var _a, _b, _c, _d, _e;
            return {
                defaultWidth: _this.getDefaultWidth(),
                growToFit: _this.isGrowToFitEnabled(),
                columnAutoresizeOption: _this.getDefaultWidthFromProps(_this.props),
                widths: _this.getColumnWidths(_this.props),
                isAltKeyPressed: _this.internal.isAltKeyPressed,
                isMetaOrCtrlKeyPressed: _this.internal.isMetaOrCtrlKeyPressed,
                // use clientWidth of the viewport container to accommodate for vertical scrollbars if needed
                clientWidth: (_c = (_b = (_a = _this.containerRef) === null || _a === void 0 ? void 0 : _a.getElementsByClassName("ag-body-viewport")[0]) === null || _b === void 0 ? void 0 : _b.clientWidth) !== null && _c !== void 0 ? _c : 0,
                containerRef: _this.containerRef,
                separators: (_e = (_d = _this.props) === null || _d === void 0 ? void 0 : _d.config) === null || _e === void 0 ? void 0 : _e.separators,
                onColumnResized: _this.props.onColumnResized,
            };
        };
        var execution = props.execution, config = props.config;
        _this.state = {
            readyToRender: false,
            columnTotals: cloneDeep(sanitizeDefTotals(execution.definition)),
    ...
    CorePivotTableAgImpl.prototype.componentDidUpdate = function (prevProps) {
        var _a, _b, _c, _d, _e;
     ...
     *   data.
     */
    CorePivotTableAgImpl.prototype.isReinitNeeded = function (prevProps) {
        var drillingIsSame = isEqual(prevProps.drillableItems, this.props.drillableItems);
        if (!drillingIsSame) {
            // eslint-disable-next-line no-console
        ...
        if (!this.internal.table) {
            ...
            if (this.internal.initializer) {
                var initializeForSameExec = this.internal.initializer.isSameExecution(this.props.execution);
                ...
                var prepExecutionSame = this.props.execution.fingerprint() === prevProps.execution.fingerprint();
     ...
     * @internal
     */
    CorePivotTableAgImpl.prototype.shouldRefreshHeader = function (props, prevProps) {
        var propsRequiringAgGridRerender = [
            function (props) { var _a; return (_a = props === null || props === void 0 ? void 0 : props.config) === null || _a === void 0 ? void 0 : _a.menu; },
        ];
        return propsRequiringAgGridRerender.some(function (propGetter) { return !isEqual(propGetter(props), propGetter(prevProps)); });
    };
    /**
     ...
     * @internal
     */
    CorePivotTableAgImpl.prototype.alertParentWrapperMissingHeight = function () {
        var _a, _b;
        var parentWrapper = (_a = document.getElementById(this.pivotTableId)) === null || _a === void 0 ? void 0 : _a.parentElement;
        var parentHeight = (_b = parentWrapper === null || parentWrapper === void 0 ? void 0 : parentWrapper.offsetHeight) !== null && _b !== void 0 ? _b : 0;
        if (parentHeight < 20) {
            // eslint-disable-next-line no-console
    ...
    CorePivotTableAgImpl.prototype.handleMouseDown = function (e) {
        var _a, _b, _c;
    ...
    CorePivotTableAgImpl.prototype.renderLoading = function () {
        var _a, _b, _c, _d, _e;
        var _f = this.props, LoadingComponent = _f.LoadingComponent, theme = _f.theme;
    ...
    CorePivotTableAgImpl.prototype.render = function () {
        var ErrorComponent = this.props.ErrorComponent;
        var _a = this.state, desiredHeight = _a.desiredHeight, error = _a.error;
        if (error) {
         ...
         */
        var shouldRenderLoadingOverlay = (this.isColumnAutoresizeEnabled() || this.isGrowToFitEnabled()) && !this.state.resized;
        return (React.createElement("div", { className: "gd-table-component", style: style, id: this.pivotTableId, onMouseDown: this.handleMouseDown },
            React.createElement("div", { className: "gd-table ag-theme-balham s-pivot-table", style: style, ref: this.setContainerRef },
                React.createElement(AgGridReact, __assign({}, this.internal.gridOptions, { modules: AllCommunityModules })),
    ...
    return CorePivotTableAgImpl;
}(React.Component));
export { CorePivotTableAgImpl };
var CorePivotTableWithIntl = injectIntl(withTheme(CorePivotTableAgImpl));
/**
 * @internal
 */
export var CorePivotTable = function (props) { return (React.createElement(ThemeContextProvider, { theme: props.theme || {}, themeIsLoading: false },
    React.createElement(IntlWrapper, { locale: props.locale },
        React.createElement(CorePivotTableWithIntl, __assign({}, props))))); };
